...
public class DP3  extends ExcelUtils{
	...
	for (j = 0; j < ExcelUtils.ExcelWSheet.getPhysicalNumberOfRows(); j++) {    			
		...
		// read values form EXCEL;    	     
			  ...
			  int TownhomeorRowHome =                         (int)                      ExcelUtils.getNumericCellValue(a, 24);
	     ...
	     RestAssured.baseURI = API_Constant_Values.URL_API_BETA;
                 Response response=
		  given()
		       .header("Content","text/xml")
	               ...
	               .body("<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:v2=\"http://www.thig.com/webservices/policy/external/v2\">\n"
	               		+ "   <soapenv:Header>\n"
	               		+ "      <v2:RequestHeader>\n"
	               		+ "         <v2:ApiKey>"+API_Constant_Values.API_KEY_BETA+"</v2:ApiKey>\n"
	               		+ "      </v2:RequestHeader>\n"
	               		+ "   </soapenv:Header>\n"
	               		+ "   <soapenv:Body>\n"
	               		+ "      <v2:D3RateRequest>\n"
